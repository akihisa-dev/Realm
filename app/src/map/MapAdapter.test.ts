...
describe("RealmMapAdapter", () => {
  ...
  it("creates a bounded world view and disposes its OpenLayers target", () => {
    ...
    const fittedResolution = resolutionForFittingExtent([-180, -90, 180, 90], [640, 480]);
    ...
    adapter.updateSize();
    expect(adapter.getZoom()).toBe(3);
    const resizedFitResolution = resolutionForFittingExtent([-180, -90, 180, 90], [900, 400]);
    ...
    expect(resizedFitExtent[0]).toBeLessThanOrEqual(WORLD_EXTENT[0] + 1e-6);
    expect(resizedFitExtent[1]).toBeLessThanOrEqual(WORLD_EXTENT[1] + 1e-6);
    expect(resizedFitExtent[2]).toBeGreaterThanOrEqual(WORLD_EXTENT[2] - 1e-6);
    expect(resizedFitExtent[3]).toBeGreaterThanOrEqual(WORLD_EXTENT[3] - 1e-6);
    expect(view.getZoom()).toBeCloseTo(view.getMinZoom());
    adapter.getMap().setSize([2000, 1400]);
    adapter.updateSize();
    expect(view.getResolution()).toBeCloseTo(resolutionForFittingExtent([-180, -90, 180, 90], [2000, 1400]));
    expect(adapter.getZoom()).toBe(1);
    adapter.dispose();
    adapter.dispose();
    expect(adapter.getMap().getTarget()).toBeNull();
