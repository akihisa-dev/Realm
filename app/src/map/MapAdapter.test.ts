import { CELL_PAINT_RANGE_MAX, CELL_GRID_CELL_COUNT, WORLD_EXTENT, RealmMapAdapter, assertGeometryWithinWorld, availableViewportSize, cellPaintRadiusForRange, cellCenter, cellId, cellIdsWithinPaintPath, cellIdsWithinPaintPosition, cellPolygon, isGeometryWithinWorld, resolutionForFillingExtent, resolutionForFittingExtent, selectFeatureIdsWithinLasso } from "./MapAdapter";
...
describe("RealmMapAdapter", () => {
  ...
  it("uses a fill resolution so the grid covers a 4:3 viewport", () => {
    expect(resolutionForFillingExtent([-180, -90, 180, 90], [1700, 1275])).toBeCloseTo(180 / 1275);
    expect(resolutionForFillingExtent([-180, -90, 180, 90], [0, 900])).toBeNaN();
  });
  ...
  it("validates grid options and keeps deterministic hex edges outside semantic selection", () => {
    ...
    expect(insideStyle.getImage()).toBeInstanceOf(CircleStyle); expect((insideStyle.getImage() as CircleStyle).getFill()?.getColor()).toBe("rgba(32, 48, 64, 0.32)"); expect((insideStyle.getImage() as CircleStyle).getRadius()).toBeCloseTo(0.9375);
    expect(outsideStyle.getStroke()?.getLineDash()).toEqual([1, 3]); expect(outsideStyle.getStroke()?.getLineCap()).toBe("round");
  ...
  it("creates a bounded world view and disposes its OpenLayers target", () => {
    ...
    expect(insideGridLayer.getSource()?.getFeatures()[0]?.getGeometry()).toBeInstanceOf(MultiPoint); expect((insideGridLayer.getSource()?.getFeatures()[0]?.getGeometry() as MultiPoint).getPoints()).toHaveLength(1); expect(insideGridStyle.getImage()).toBeInstanceOf(CircleStyle); expect((insideGridStyle.getImage() as CircleStyle).getFill()?.getColor()).toBe("rgba(209, 215, 220, 0.32)");
    ...
    const filledResolution = resolutionForFillingExtent([-180, -90, 180, 90], [640, 480]);
    expect(view.getResolution()).toBeCloseTo(filledResolution);
    expect(view.getZoom()).toBeCloseTo(view.getMinZoom());
    const initialFillExtent = view.calculateExtent([640, 480]);
    expect(initialFillExtent[0]).toBeGreaterThan(WORLD_EXTENT[0]);
    expect(initialFillExtent[1]).toBeCloseTo(WORLD_EXTENT[1]);
    expect(initialFillExtent[2]).toBeLessThan(WORLD_EXTENT[2]);
    expect(initialFillExtent[3]).toBeCloseTo(WORLD_EXTENT[3]);
    ...
    expect(constrainedCenter?.[1]).toBeCloseTo(0);
    ...
    adapter.updateSize();
    expect(adapter.getZoom()).toBe(3);
    const resizedFillResolution = resolutionForFillingExtent([-180, -90, 180, 90], [900, 400]);
    const resizedFillZoom = view.getZoomForResolution(resizedFillResolution);
    expect(resizedFillZoom).not.toBeUndefined();
    expect(view.getResolution()).toBeCloseTo(view.getResolutionForZoom((resizedFillZoom ?? 0) + 2));
    adapter.setZoom(1);
    view.setCenter([0, 0]);
    const resizedFillExtent = view.calculateExtent([900, 400]);
    expect(resizedFillExtent[0]).toBeCloseTo(WORLD_EXTENT[0]);
    expect(resizedFillExtent[1]).toBeGreaterThan(WORLD_EXTENT[1]);
    expect(resizedFillExtent[2]).toBeCloseTo(WORLD_EXTENT[2]);
    expect(resizedFillExtent[3]).toBeLessThan(WORLD_EXTENT[3]);
    expect(view.getZoom()).toBeCloseTo(view.getMinZoom());
    adapter.getMap().setSize([2000, 1400]);
    adapter.updateSize();
    expect(view.getResolution()).toBeCloseTo(resolutionForFillingExtent([-180, -90, 180, 90], [2000, 1400]));
    expect(adapter.getZoom()).toBe(1);
    adapter.dispose();
    adapter.dispose();
    expect(adapter.getMap().getTarget()).toBeNull();
