...
export function LayerManager({ activeLayer, onLayerChange, onClose, disabled = false, terrainCount, regions, selectedRegionIds, selectedComponentId, regionPaintTargetId, onSelectRegion, onSelectComponent, onStartNewRegion, onAddToRegion, onSplitComponent, objects, selectedObjectIds, objectKind, objectLabel, onObjectKindChange, onObjectLabelChange, onStartObjectDraw, onSelectObject, onDeleteObject }: LayerManagerProps) {
  return (
    <aside className="layer-manager" aria-label="レイヤー管理">
      ...
      <div id={`layer-panel-${activeLayer}`} role="tabpanel" aria-labelledby={`layer-tab-${activeLayer}`} className="layer-manager-content">
        {activeLayer === "terrain" ? <section className="layer-panel" aria-labelledby="terrain-layer-heading"><div className="layer-panel-section-heading"><h3 id="terrain-layer-heading">地形</h3><span>{terrainCount}個</span></div><p className="layer-panel-help">現在の地形そのものを編集します。六角グリッドへ吸着した地形ポリゴンを管理します。</p><p className="layer-panel-empty">左のレールの「描く」または「消す」で編集できます。</p></section> : null}
        {activeLayer === "region" ? <RegionPanel regions={regions} selectedRegionIds={selectedRegionIds} selectedComponentId={selectedComponentId} regionPaintTargetId={regionPaintTargetId} disabled={disabled} onSelectRegion={onSelectRegion} onSelectComponent={onSelectComponent} onStartNewRegion={onStartNewRegion} onAddToRegion={onAddToRegion} onSplitComponent={onSplitComponent} onClose={() => undefined} embedded /> : null}
