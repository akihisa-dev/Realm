      ...
      <div className="tool-rail-tools" role="toolbar" aria-label="地図ツール">
        {isObjectKind ? <div className="tool-rail-item">
          ...
          <div className="tool-rail-item">
            <button className={`tool-rail-button${gridIsActive ? " is-active" : ""}`} type="button" aria-label="グリッド" aria-pressed={gridIsActive} aria-controls={kind === "region" ? REGION_COLOR_FLYOUT_ID : undefined} aria-expanded={kind === "region" ? regionColorFlyoutTool === "grid" : undefined} title="グリッド" onClick={(event) => toggleCellPaintTool("grid", event)}><GridFour aria-hidden="true" size={20} weight="bold" /></button>
            {regionColorFlyoutTool === "grid" ? regionColorFlyout : null}
          </div>
          <div className="tool-rail-item">
            <button className={`tool-rail-button${areaIsActive ? " is-active" : ""}`} type="button" aria-label="範囲" aria-pressed={areaIsActive} aria-controls={kind === "region" ? REGION_COLOR_FLYOUT_ID : undefined} aria-expanded={kind === "region" ? regionColorFlyoutTool === "area" : undefined} title="範囲" onClick={(event) => toggleCellPaintTool("area", event)}><Polygon aria-hidden="true" size={20} weight="bold" /></button>
