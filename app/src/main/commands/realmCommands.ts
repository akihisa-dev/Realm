...
import type { RealmBackend, RealmSnapshot, ProjectSummary, CreateFeatureInput, CreateFeaturesBatchInput, ReviseFeaturesBatchInput, ReviseFeatureInput, DeleteFeaturesBatchInput, SetFeaturesLockedInput, ApplyCellAttributesInput, CellViewportInput, ProjectSettings, ImportAssetInput, ImportAssetsBatchInput, AssetRead, FeatureProperties, MoveRegionCellsInput } from "../../shared/realmContract";
...
import { GRID_VERSION, normalizeCellIds, parseCellId, validateCellLayer, validateCellValue } from "../domain/cell";
...
export class RealmCommands implements RealmBackend {
  ...
  private setSession(session: OpenProjectSession): RealmSnapshot { this.session?.close(); this.session = session; return projectSnapshot(session); }
  ...
  async applyCellAttributes(input: ApplyCellAttributesInput): Promise<RealmSnapshot> { assertRecord(input); validateCellLayer(input.attribute); const cells = normalizeCellIds(input.cellIds); const value = validateCellValue(input.value); const session = this.current(); const before = captureState(session.database); transaction(session.database, () => { const deleteStatement = session.database.prepare("DELETE FROM cell_attributes WHERE grid_version=? AND cell_x=? AND cell_y=? AND layer=?"); const insertStatement = session.database.prepare("INSERT INTO cell_attributes(grid_version,cell_x,cell_y,layer,value) VALUES (?,?,?,?,?) ON CONFLICT(grid_version,cell_x,cell_y,layer) DO UPDATE SET value=excluded.value"); for (const [x, y] of cells) { if (value === null) deleteStatement.run(GRID_VERSION, x, y, input.attribute); else insertStatement.run(GRID_VERSION, x, y, input.attribute, value); } }); session.checkpoint(before, "cell-attributes"); return projectSnapshot(session); }
  async moveRegionCells(input: MoveRegionCellsInput): Promise<RealmSnapshot> {
    ...
    const source = input.sourceCellIds.map((id) => { if (typeof id !== "string") throw invalid("The region move is invalid."); parseCellId(id); return id.trim(); });
    const target = input.targetCellIds.map((id) => { if (typeof id !== "string") throw invalid("The region move is invalid."); parseCellId(id); return id.trim(); });
    if (new Set(source).size !== source.length || new Set(target).size !== target.length) throw invalid("The region move is invalid.");
    const axial = (id: string): [number, number] => { const [x, y] = parseCellId(id); return [x - Math.floor(y / 2), y]; };
    const origin = axial(source[0]!); const destination = axial(target[0]!); const delta: [number, number] = [destination[0] - origin[0], destination[1] - origin[1]];
    ...
    if (expected.some((id, index) => id !== target[index])) throw invalid("The region move must stay on the fixed hex grid.");
    const session = this.current(); const read = session.database.prepare("SELECT value FROM cell_attributes WHERE grid_version=? AND cell_x=? AND cell_y=? AND layer='region'"); const terrainRead = session.database.prepare("SELECT 1 FROM cell_attributes WHERE grid_version=? AND cell_x=? AND cell_y=? AND layer='terrain'");
    const rows = source.map((id) => { const [x, y] = parseCellId(id); return read.get(GRID_VERSION, x, y) as { value?: unknown } | undefined; });
    if (rows.some((row) => !row || typeof row.value !== "string")) throw new RealmError("not_found", "The region to move was not found.");
    const color = String(rows[0]!.value); if (rows.some((row) => String(row!.value) !== color)) throw invalid("Only one region color can be moved at a time.");
    const sourceSet = new Set(source); const clippedTarget = target.filter((id) => { const [x, y] = parseCellId(id); return Boolean(terrainRead.get(GRID_VERSION, x, y)); });
    if (clippedTarget.some((id) => { const [x, y] = parseCellId(id); return Boolean(read.get(GRID_VERSION, x, y) && !sourceSet.has(id)); })) throw invalid("The target contains another region.");
    if (source.every((id, index) => id === target[index])) return projectSnapshot(session);
    const before = captureState(session.database); transaction(session.database, () => { const remove = session.database.prepare("DELETE FROM cell_attributes WHERE grid_version=? AND cell_x=? AND cell_y=? AND layer='region'"); const insert = session.database.prepare("INSERT INTO cell_attributes(grid_version,cell_x,cell_y,layer,value) VALUES (?,?,?,?,?) ON CONFLICT(grid_version,cell_x,cell_y,layer) DO UPDATE SET value=excluded.value"); for (const id of source) { const [x, y] = parseCellId(id); remove.run(GRID_VERSION, x, y); } for (const id of clippedTarget) { const [x, y] = parseCellId(id); insert.run(GRID_VERSION, x, y, "region", color); } }); session.checkpoint(before, "move-region-cells"); return projectSnapshot(session);
  }
  ...
  async closeProject(): Promise<void> { this.session?.close(); this.session = null; }
