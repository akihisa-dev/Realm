...
import { createFeaturesBatch as createFeaturesBatchCommand, deleteFeaturesBatch as deleteFeaturesBatchCommand, reviseFeaturesBatch as reviseFeaturesBatchCommand, setFeaturesLocked as setFeaturesLockedCommand } from "./featureCommands";
...
export class RealmCommands implements RealmBackend {
  ...
  async createFeature(input: CreateFeatureInput): Promise<RealmSnapshot> { return createFeaturesBatchCommand(() => this.current(), { features: [input] }); }
  async createFeaturesBatch(input: CreateFeaturesBatchInput): Promise<RealmSnapshot> { return createFeaturesBatchCommand(() => this.current(), input); }
  async reviseFeature(input: ReviseFeatureInput): Promise<RealmSnapshot> { return reviseFeaturesBatchCommand(() => this.current(), { features: [input] }); }
  async reviseFeaturesBatch(input: ReviseFeaturesBatchInput): Promise<RealmSnapshot> { return reviseFeaturesBatchCommand(() => this.current(), input); }
  async deleteFeature(input: { id: string }): Promise<RealmSnapshot> { return deleteFeaturesBatchCommand(() => this.current(), { ids: [input.id] }); }
  async deleteFeaturesBatch(input: DeleteFeaturesBatchInput): Promise<RealmSnapshot> { return deleteFeaturesBatchCommand(() => this.current(), input); }
  async setFeaturesLocked(input: SetFeaturesLockedInput): Promise<RealmSnapshot> { return setFeaturesLockedCommand(() => this.current(), input); }
