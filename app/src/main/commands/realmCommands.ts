...
import type { RealmBackend, RealmSnapshot, ProjectSummary, CreateFeatureInput, CreateFeaturesBatchInput, ReviseFeaturesBatchInput, ReviseFeatureInput, DeleteFeaturesBatchInput, SetFeaturesLockedInput, ApplyCellAttributesInput, CellViewportInput, ProjectSettings, ImportAssetInput, ImportAssetsBatchInput, AssetRead, FeatureProperties } from "../../shared/realmContract";
...
import { normalizeCellIds, validateCellLayer, validateCellValue } from "../domain/cell";
...
export class RealmCommands implements RealmBackend {
  ...
  async applyCellAttributes(input: ApplyCellAttributesInput): Promise<RealmSnapshot> { assertRecord(input); validateCellLayer(input.attribute); const cells = normalizeCellIds(input.cellIds); const value = validateCellValue(input.value); const session = this.current(); const before = captureState(session.database); transaction(session.database, () => { const deleteStatement = session.database.prepare("DELETE FROM cell_attributes WHERE grid_version=1 AND cell_x=? AND cell_y=? AND layer=?"); const insertStatement = session.database.prepare("INSERT INTO cell_attributes(grid_version,cell_x,cell_y,layer,value) VALUES (1,?,?,?,?) ON CONFLICT(grid_version,cell_x,cell_y,layer) DO UPDATE SET value=excluded.value"); for (const [x, y] of cells) { if (value === null) deleteStatement.run(x, y, input.attribute); else insertStatement.run(x, y, input.attribute, value); } }); session.checkpoint(before, "cell-attributes"); return projectSnapshot(session); }
