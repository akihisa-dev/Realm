...
export class RealmCommands implements RealmBackend {
  ...
  async moveRegionCells(input: MoveRegionCellsInput): Promise<RealmSnapshot> {
    ...
    const session = this.current(); const read = session.database.prepare("SELECT value FROM cell_attributes WHERE grid_version=? AND cell_x=? AND cell_y=? AND layer='region'");
    ...
    const sourceSet = new Set(source); if (target.some((id) => { const [x, y] = parseCellId(id); return Boolean(read.get(GRID_VERSION, x, y) && !sourceSet.has(id)); })) throw invalid("The target contains another region.");
    if (source.every((id, index) => id === target[index])) return projectSnapshot(session);
    const before = captureState(session.database); transaction(session.database, () => { const remove = session.database.prepare("DELETE FROM cell_attributes WHERE grid_version=? AND cell_x=? AND cell_y=? AND layer='region'"); const insert = session.database.prepare("INSERT INTO cell_attributes(grid_version,cell_x,cell_y,layer,value) VALUES (?,?,?,?,?) ON CONFLICT(grid_version,cell_x,cell_y,layer) DO UPDATE SET value=excluded.value"); for (const id of source) { const [x, y] = parseCellId(id); remove.run(GRID_VERSION, x, y); } for (const id of target) { const [x, y] = parseCellId(id); insert.run(GRID_VERSION, x, y, "region", color); } }); session.checkpoint(before, "move-region-cells"); return projectSnapshot(session);
  }
