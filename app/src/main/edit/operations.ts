...
export type CellRow = { id: number; gridVersion: number; cellX: number; cellY: number; layer: CellAttribute; value: string; regionId: string | null };
...
export function captureState(db: DatabaseSync): PersistentState {
  ...
  return {
    ...
    cells: rows(db, "SELECT id,grid_version AS gridVersion,cell_x AS cellX,cell_y AS cellY,layer,value,region_id AS regionId FROM cell_attributes ORDER BY id", (row) => ({ id: Number(row.id), gridVersion: Number(row.gridVersion), cellX: Number(row.cellX), cellY: Number(row.cellY), layer: String(row.layer) as CellAttribute, value: String(row.value), regionId: row.regionId === null || row.regionId === undefined ? null : String(row.regionId) })),
  };
}
export function restoreState(db: DatabaseSync, state: PersistentState): void {
  transaction(db, () => {
    ...
    const cell = db.prepare("INSERT INTO cell_attributes(id,grid_version,cell_x,cell_y,layer,value,region_id) VALUES (?,?,?,?,?,?,?)"); for (const row of state.cells) cell.run(row.id, row.gridVersion, row.cellX, row.cellY, row.layer, row.value, row.regionId);
  });
}
