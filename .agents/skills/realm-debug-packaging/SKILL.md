...
description: RealmのElectron ForgeによるApple Silicon macOS .app・DMG、bundle metadata、arm64実行file、resource、checksum、SBOM・noticesの静的検査と障害を扱う。配布build失敗、成果物欠落、不要file混入、package検査に使い、tag・Releaseはrealm-releaseを優先する。
---

# Realm Packaging Debug

1. 調査だけなら成果物を作り直さず、OS、command、最初の失敗を特定する。
2. package、Forge config、Vite/build・check・asset script、release docsを読む。
3. renderer build、Electron Forge bundle、DMG、metadata、release-assetsを分けて再現する。
