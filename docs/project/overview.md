...
- Drawing or erasing applies the complete set of valid hexagonal cells touched by one pointer stroke in one transaction and one undo step. Before pressing, the fixed brush footprint is shown as a temporary dashed preview at the pointer; the preview is never persisted. The active terrain tool remains selected for consecutive strokes; Escape cancels the current selection.
