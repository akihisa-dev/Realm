...
| Region | One or more hexagonal grid cells carrying the same current display color, rendered independently over the terrain. |
...
- Region drawing uses a freehand enclosure to select the valid hexagonal cells inside it, then applies the selected color to that complete cell set in one transaction and one undo step without changing the terrain cells beneath it. The enclosure is transient input only: saved region fills and boundaries combine same-colored adjacent cells into deterministic smooth shell/hole polygons rather than exposing internal seams. Newly added or recolored region cells briefly fade into their final translucent style; reduced-motion presentation applies the final style immediately. The animation is transient renderer state and does not add saved history.
