...
左ツールレールには、地形・領域レイヤーでは「グリッド描画」「範囲描画」「消す」「掴む」を、領域レイヤーでは追加で「シェイピング」を、オブジェクトレイヤーでは「描く」「消す」「掴む」をアイコンだけで表示します。最下部には「新規作成」を配置します。新規作成では現在の世界を残したまま、空の新しい世界を開きます。
「グリッド描画」と「範囲描画」は地形と領域で共通です。前者は六角形セルを押したまま塗り、後者は自由線で囲んだ範囲のセルを一括で塗り、active layerに応じたレイヤーだけを変更します。
...
| グリッド描画（Grid draw） | 固定六角形グリッド上でセルを押したまま塗り、active layerの地形または領域ポリゴンだけを変更する操作 |
| 領域レイヤー（Region layer） | 名前付きまたは色付きの領域と、グリッドに吸着した領域ポリゴン部分。地形から独立し、オブジェクトレイヤーではない |
| 範囲描画（Area draw） | 自由な囲い込み選択で内側のセルを一括して塗り、active layerの地形または領域ポリゴンだけを変更する操作 |
...
| 描く（Object draw） | オブジェクトレイヤーで種類ごとの点またはポリゴンを配置する操作 |
...
- 左ツールレールには地形・領域で共通の「グリッド描画」「範囲描画」「消す」「掴む」を表示し、領域レイヤーでは「シェイピング」も選べる。オブジェクトレイヤーでは「描く」「消す」「掴む」を表示する。最下部の「新規作成」で現在の世界を残したまま空の新しい世界を開ける
- 領域の「シェイピング」は、クリックした論理領域のうち地形のない部分だけを削り、地形上の領域セルと領域IDを維持する。結果は1回の保存・undo単位として扱う
- グリッド描画と範囲描画はactive layerの地形または領域ポリゴンだけを変更する。消しゴムは地形、領域、オブジェクトのうちactive layerだけを対象にし、オブジェクトの配置、移動、選択、削除もオブジェクトだけを変更する
