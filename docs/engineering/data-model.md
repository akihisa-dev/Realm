...
The active editor uses a fixed 128 by 73 EPSG:4326 odd-row-offset grid of regular point-topped hexagons. Transient cell identity is `x:y`, with `x` from 0 through 127 and `y` from 0 through 72. Each selection is converted to the exposed boundary rings of its hex cells and saved as a grid-snapped `Polygon`; boundary cells are clipped to the bounded world. The dedicated grid and cell IDs are renderer/editor aids only. OpenLayers objects and transient cell rows are never the storage source of truth.
