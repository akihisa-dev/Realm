...
The active editor uses a fixed 128 by 73 EPSG:4326 odd-row-offset grid of regular point-topped hexagons. Transient cell identity is `x:y`, with `x` from 0 through 127 and `y` from 0 through 72. Paint and enclosure selections are converted to the exposed boundary rings of their hex cells and saved as grid-snapped `Polygon` rows; boundary cells are clipped to the bounded world. Grab edits begin with exact saved Polygon geometry. During a boundary pull, the raw geometry stays renderer-only while the affected grid cells are projected and shown immediately; only the released geometry is normalized back to these rings and saved. A substantial pull includes only the positive-area cells added by the moved boundary and a bounded local neighborhood on that side, so the extension tapers without changing unrelated edges; small adjustments remain on the original cells. The dedicated grid and cell IDs are renderer/editor aids only. OpenLayers objects and transient cell rows are never the storage source of truth.
