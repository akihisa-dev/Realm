...
- The object manager derives one region object from each persistent region ID and derives its region chunks by six-neighbor connectivity. Region objects and chunks are renderer/editor views, not additional SQLite entities; adding, merging, and separating them rewrites the affected region cell rows through the existing transactional cell-attribute command.
