...
`GrabHoverController` derives terrain and region boundary cells in explicit `grab` mode without changing stored attributes, accepts a press anywhere inside the visible boundary cell, and marks that cell with a dashed renderer-only affordance while changing the canvas cursor to `grab`. `RegionGrabController` is installed only for explicit `grab`, so terrain drawing and region enclosure interactions retain every primary-button gesture; a grab boundary press resizes the selected terrain or region cell mass, while an interior region press moves the whole logical region.
