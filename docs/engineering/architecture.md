...
- SQLite cannot adopt a raw staging fd; each connection therefore combines held-fd/path dev-ino checks with `SQLITE_FCNTL_HAS_MOVED` immediately before writes and publication. A mismatch fails closed and leaves foreign files untouched.
