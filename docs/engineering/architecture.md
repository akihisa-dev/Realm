...
- React owns the transient hex-cell selection used by paint/erase, region color, viewport, tool mode including grab and shaping, the read-only map preview state, the shared header-controlled thickness range used by terrain painting and erasing, object-manager open state, selected logical regions, selected region part, expanded object-manager rows, and serialized mutation state. It owns the canonical `MapShape[]` draft until the main-process replacement has completed.
