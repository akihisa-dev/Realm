...
- React owns transient hex-cell selection, region color, viewport, tool mode including grab, draw-paint range, and serialized mutation state.
...
The active editor reads the current project snapshot and terrain and region cell attributes; paints or clears a validated terrain cell batch; paints a validated color-valued region cell batch with one persistent region ID per completed stroke; moves every cell of one validated region ID through one coarse IPC transaction, while the renderer masks destination cells outside terrain and the persistence layer keeps stationary regions while omitting moving cells that overlap them; derives any disconnected remainder as separate six-connected display shapes without splitting the ID; resizes an edge through region cell-attribute updates that retain the ID; and performs undo or redo. On launch, React restores the open world, opens the first app-library world, or creates `無題の世界` when the library is empty, then enters the editor directly. Import, file export, project-name editing, presentation settings, feature editing, asset commands, and unsupported cell layers remain compatibility infrastructure without an active editor entry. Cell commands accept complete bounded `x:y` identifier sets.
