...
- React owns transient hex-cell selection, region color, viewport, tool mode, draw-paint range, and serialized mutation state.
- OpenLayers owns only rendering and interaction objects derived from terrain cell rows and region features; map objects are not an additional source of truth. Completed terrain gestures cross the renderer boundary as bounded stable cell identifiers, while completed region gestures cross as bounded GeoJSON polygons.

The active editor reads the current project snapshot, visible region features, and terrain cell attributes; paints or clears a validated cell batch; creates a validated colored region polygon; and performs undo or redo. On launch, React restores the open world, opens the first app-library world, or creates `無題の世界` when the library is empty, then enters the editor directly. Import, file export, project-name editing, presentation settings, other feature editing, asset commands, and non-terrain cell layers remain compatibility infrastructure without an active editor entry. Terrain cell commands accept complete bounded `x:y` identifier sets.
