...
左ツールレールには、active layerに共通する3つのアイコンだけを表示します。
「描く」と「消す」はactive layerに応じて地形、領域、またはオブジェクトのハンドラーへ振り分け、「掴む」は地形・領域では形状操作、オブジェクトでは選択と移動へ振り分けます。
領域の色と消去対象はレールから開くflyoutで扱い、オブジェクトの種類（`city`、`text`、`mountain`、`forest`）とラベルは右パネルで選びます。
