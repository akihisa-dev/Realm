...
The coarse IPC surface lists, creates, opens, saves, closes, imports, and exports library worlds; writes validated PNG/JPEG/PDF artifact bytes; reads the current project snapshot; updates bounded canvas, palette, grid, and export settings; creates, revises, deletes, locks, or batch-mutates validated features; imports, reads, and deletes project-embedded image assets individually or in bounded packs; applies one cell-attribute batch; reads current cell attributes for an optional grid viewport; and performs undo or redo. Automatic save replaces the editable world name in one Rust transaction. Feature commands accept complete validated GeoJSON geometry, while cell commands accept bounded stable IDs rather than SQL-shaped or per-coordinate calls. Project snapshots contain asset manifests only; one explicit read command returns one bounded asset's bytes.
