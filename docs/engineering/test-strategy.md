...
| React/UI | three right-panel tabs, active-layer state, non-active-layer editing lock, shared draw entry with layer-specific behavior, layer-specific sidebar operations and eraser labels, shared pan/zoom controls, preview read-only state, object kind/label/placement controls, object list selection/deletion, region management, locked-object controls, optimistic save/failure recovery, stale request rejection, and undo/redo state |
