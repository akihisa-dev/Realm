...
| React/UI | 右パネルの3タブ、active layer状態、非active layerの編集ロック、共通3アイコンと領域専用シェイピングを持つツールレール、active layerに応じた描く・消す・掴む・シェイピングの振り分けと消去対象、共有パンとズーム、読み取り専用プレビュー状態、オブジェクト種類・ラベル・配置操作、オブジェクト一覧の選択と削除、領域管理、ロック済みオブジェクト操作、楽観的保存と失敗回復、古い要求の拒否、undo/redo状態 |
