...
| React/UI | library create/open/import, serialized automatic save, newer-draft preservation, PNG transparency/JPEG quality/PDF and transfer actions, persisted palette/grid/canvas/export settings, feature tools and appearance controls, polygon-hole editing, multi-selection group edits, trace/frame/scale controls, seeded scatter, grouped embedded asset management, brush attribute/size/paint/erase controls, project-identity selection reset, stale request rejection, and undo/redo state |
| OpenLayers adapter | bounded and finite EPSG:4326 geometry, freehand/vertex drawing, smoothing and angle snapping, lasso/keyboard nudge and locked-feature exclusion, snapshot replacement, palette-aware textured point/line/polygon presentation, trace/frame/scale and embedded-symbol rendering, geographic/square/hex grids, lazy cell creation, pressure-aware brush preview, round brush stamps, gap-free thick paths, world-edge rejection, cached property-sensitive styles, cancellation, pan/zoom, bounded configured raster export and state restoration, listener cleanup, and idempotent disposal |
