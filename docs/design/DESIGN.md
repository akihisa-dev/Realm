...
The startup concept defines composition and density. Visible controls remain code-native. The primary rail exposes only move, draw terrain, and erase terrain. The sidebar lists terrain polygons and edits their name, geometry, visibility, opacity, lock, and order. Non-terrain compatibility rows never appear in the editor.
