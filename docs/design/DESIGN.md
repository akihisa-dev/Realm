...
- A world-open editor provides pan, zoom, terrain drawing and selection, undo, redo, library return, PNG/JPEG/PDF export, and transfer export. Valid edits save automatically.
...
The editor uses one native overlay title bar with a compact file toolbar, a narrow three-item primary rail, a dominant OpenLayers canvas, and compact map controls. The rail continues to the bottom edge; no secondary project sidebar is shown. Do not draw a second set of macOS traffic lights in React. Default zoom and center are part of the map adapter contract and must not be persisted as project data.

## Allowed initial copy

The startup state uses `Realm`, `創作世界の地図を、アプリ内で安全に管理。`, `新しい世界を作成`, and `移行データを読み込む`, together with the library worlds. The editor uses the world name, `ライブラリ`, `PNG`, `PDF`, `移行データ`, `自動保存中…`, `自動保存済み`, `元に戻す`, `やり直す`, `移動`, `地形を描く`, `地形を消す`, and the zoom value. Controls for terrain lists, creation metadata, drawing configuration, presentation settings, unavailable layers, or asset management are not shown.
