...
Visible controls remain code-native. The radial map palette exposes the terrain drawing/thickness control, region color control, eraser, and an accessible `グラブ` button. There is no startup screen, terrain list, creation form, persistent drawing-range panel, or presentation-settings sidebar. Selecting `地形を描く（太さ調整）` activates terrain drawing and opens an accessible 1–5 cell thickness flyout. Selecting `領域` activates freehand enclosure selection and opens an accessible native color input; the chosen color remains transient until a completed enclosure is saved. The enclosure is only an input guide: its interior hex cells become the saved region, while the completed presentation derives separate boundaries for each terrain-backed six-connected component. Selecting `消しゴム` activates layer deletion and opens its `地形削除` / `領域削除` toggle buttons plus the 1–5 cell thickness control; `地形削除` is the default and clears only the terrain layer, while `領域削除` clears only the region layer. The deletion target definitions remain grouped so future eraser targets can be added without changing the pointer workflow. Selecting `グラブ` treats the pressed same-color, hex-connected region cells as one mass, previews its fixed-grid translation clipped to terrain cells, removes destination cells outside terrain when saved, and keeps any resulting disconnected components as independent derived shapes. The preview hides the stale smooth mass and shows exact destination cells until the grab is released. World-edge or occupied-terrain destinations are rejected. The flyouts remain outside the palette, clamp to the viewport, and close on the documented exclusive, outside, Escape, or blur actions. Terrain drawing, region enclosure, erasing, and grab retain temporary previews and cancellation guarantees. A primary-button map action used to dismiss the palette is consumed and cannot also edit or pan the map. Terrain and region cell rows appear together; compatibility feature rows and other unsupported compatibility data remain hidden.
