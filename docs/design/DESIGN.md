...
左railは順序を固定した「描く」「消す」「掴む」と新規作成footerです。グリッド描画・範囲描画は「描く」の設定、シェイピングは選択regionの右パネル操作に置きます。地図移動とzoom gestureはrail項目ではありません。
...
- 左ツールレールには、描く内容にかかわらず「描く」「消す」「掴む」を同じ順序で表示する。最下部に「新規作成」を置き、描く内容と方法、領域操作、オブジェクトの種類と一覧操作は右のレイヤー管理で扱う
...
主役のOpenLayersキャンバスは、常に「描く」「消す」「掴む」と下部の新規作成アクションを置く左ツールレールと、右レイヤー管理の間に置きます。
...
左ツールレールでは常に`描く`、`消す`、`掴む`を同じ順序で表示し、最下部に`新規作成`を置きます。右パネルで描く内容と方法を選び、選択した領域を地形へ合わせます。新規作成は現在の世界を残したまま空の新しい世界を開きます。
領域の色と消す対象は描画または消去アイコンを選んだときのflyoutで確認でき、オブジェクトの種類は右パネルで選びます。
