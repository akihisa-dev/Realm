...
- 左ツールレールには、active layerに応じて動作を変える共通の「描く」「消す」「掴む」を表示し、領域レイヤーでは追加で「シェイピング」を表示する。最下部に「新規作成」を置く。領域の色と削除対象は必要時のflyoutで、オブジェクトの種類と一覧操作は右のレイヤー管理で扱う。新規作成は現在の世界を残したまま空の新しい世界を開く
...
主役のOpenLayersキャンバスは、共通の3つの編集アイコンと、領域レイヤーで表示するシェイピング操作、下部の新規作成アクションを置く左ツールレールと、右レイヤー管理の間に置きます。
...
左ツールレールでは、`描く`、`消す`、`掴む`を編集用のアイコンだけで使い、領域レイヤーでは`シェイピング`も表示し、最下部に`新規作成`を置きます。描く操作と消す操作の対象はactive layerに応じて変わり、シェイピングはクリックした領域を地形の形に合わせます。新規作成は現在の世界を残したまま空の新しい世界を開きます。
領域の色と消す対象はアイコンを選んだときのflyoutで確認でき、オブジェクトの種類は右パネルで選びます。
