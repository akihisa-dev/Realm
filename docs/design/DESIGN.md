...
- 左ツールレールには、地形・領域レイヤーでは共通の「グリッド描画」「範囲描画」「消す」「掴む」を表示し、領域レイヤーでは追加で「シェイピング」を表示する。オブジェクトレイヤーでは「描く」「消す」「掴む」を表示する。最下部に「新規作成」を置き、領域の色と削除対象は必要時のflyoutで、オブジェクトの種類と一覧操作は右のレイヤー管理で扱う。新規作成は現在の世界を残したまま空の新しい世界を開く
- 「グリッド描画」と「範囲描画」は地形・領域で共通の入力方式であり、active layerが対象と保存結果を決める。グリッド描画は六角形セルを押したまま塗り、範囲描画は自由線で囲んだ内側のセルを一括で塗る。オブジェクトは種類ごとの点またはポリゴン配置を使い、シェイピングは領域を地形の形に合わせる
...
主役のOpenLayersキャンバスは、地形・領域で共通の4つの編集アイコン、領域レイヤーで表示するシェイピング操作、またはオブジェクトレイヤーの3つの編集アイコン、下部の新規作成アクションを置く左ツールレールと、右レイヤー管理の間に置きます。
...
左ツールレールでは、地形・領域レイヤーに`グリッド描画`、`範囲描画`、`消す`、`掴む`を、領域レイヤーに`シェイピング`を、オブジェクトレイヤーに`描く`、`消す`、`掴む`を表示し、最下部に`新規作成`を置きます。グリッド描画と範囲描画の対象はactive layerに応じて変わり、シェイピングはクリックした領域を地形の形に合わせます。新規作成は現在の世界を残したまま空の新しい世界を開きます。
