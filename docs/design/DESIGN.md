...
- 左ツールレールには、active layerに応じて動作を変える「描く」「消す」「掴む」の3つのアイコンだけを表示する。領域の色と削除対象は必要時のflyoutで、オブジェクトの種類と一覧操作は右のレイヤー管理で扱う
...
主役のOpenLayersキャンバスは、3つのアイコンだけを置く左ツールレールと右レイヤー管理の間に置きます。
...
左ツールレールでは、`描く`、`消す`、`掴む`をアイコンだけで使います。描く操作と消す操作の対象はactive layerに応じて変わります。
